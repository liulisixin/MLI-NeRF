...
def inside_camera_view(intr, pose, image_size, pts, z_min=0.00):
    pts_cam = world2cam(pts, pose)

    z_in_range = pts_cam[..., 2] > z_min

    pixel_coordinate = cam2img(pts_cam, intr)
    points_2d = pixel_coordinate[..., :2] / pixel_coordinate[..., 2:]

    x_min, x_max = 0.5, image_size[0] - 0.5
    y_min, y_max = 0.5, image_size[1] - 0.5

    in_range = (points_2d[..., 0] >= x_min) & (points_2d[..., 0] <= x_max) & (points_2d[..., 1] >= y_min) & (
                points_2d[..., 1] <= y_max)

    in_range = in_range & z_in_range
