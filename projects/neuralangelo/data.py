...
class Dataset(base.Dataset):
    ...
    def __init__(self, cfg, is_inference=False, is_test=False):
        ...
        if 'annotation' in cfg_data[self.split]:
            meta_fname = cfg_data[self.split].annotation
        else:
            meta_fname = f"{cfg_data.root}/{self.split}_transforms.json"
        with open(meta_fname) as file:
            self.meta = json.load(file)
            if not hasattr(self.meta, "sphere_center"):
                self.meta["sphere_center"] = [0.0, 0.0, 0.0]
            if not hasattr(self.meta, "sphere_radius"):
                self.meta["sphere_radius"] = 1.0
            if hasattr(cfg_data[self.split], 'use_light_index'):
                use_light_index = cfg_data[self.split].use_light_index
                filter_frames = [x for x in self.meta["frames"] if x['light_index'] in use_light_index]
                self.meta["frames"] = filter_frames
        self.list = self.meta["frames"]
        # get raw size
        _, img_size_raw = self.get_image(0)
        self.raw_W, self.raw_H = img_size_raw
